...
* Your CV can be written using either Markdown ([preview](https://rsnaudon.github.io/cv/)) or generated via JSON ([preview](https://rsnaudon.github.io/cv-json/)) and the layouts are slightly different. You can update the path to the one being used in `_data/navigation.yml` with the JSON formatted CV being hidden by default.
